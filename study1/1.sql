--mysql起動
mysql.server start --mysql停止
mysql.server stop --プロセスの確認・終了
ps aux | grep "mysql" sudo kill pid --初期設定
mysql_secure_installation --mysql　パスなしログイン
mysql.server start --skip-grant-tables
--mysqlログイン
mysql - u root - p --mysqlユーザー確認
select
	Host,
	User,
	Password
...
SET
	user = 'root',
	host = 'localhost',
	password = Password('silversun0424');
...
SET
	user = 'root',
	host = 'localhost',
	authentication_string = password('silversun0424'),
	Select_priv = 'y',
	Insert_priv = 'y',
	Update_priv = 'y',
	Delete_priv = 'y',
...
--パスワードの設定
set
	password for 'ユーザー名' @'ホスト名' = password('パスワード');

set
	password for 'root' @'localhost' = password('silversun0424');
...
values
	('マーク・トウェイン', 'トム・ソーヤの冒険', 'フィクション', '1876');

insert into
	classics(author, title, type, year)
values
	('ジェーン・オースティン', '傲慢と偏見', 'フィクション', '1811');
